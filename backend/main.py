...
async def online_all(limit_per_realm: Optional[int] = 200, page: int = 1, page_size: int = 50):
	...
	# race -> faction mapping (Alliance vs Horde)
	alliance_races = {1, 3, 4, 7, 11}
	horde_races = {2, 5, 6, 8, 10}
	...
	async def fetch_chars_for_realm(r):
		...
		for row in rows:
			...
			# determine faction: 1 = Horde, 2 = Alliance, 0 = unknown
...
async def top_pvp(realm_id: Optional[int] = None, limit: int = 100):
	"""Return top PvP killers per realm (by characters.totalkill).

	If `realm_id` is provided, returns only that realm; otherwise returns all realms.
	"""
